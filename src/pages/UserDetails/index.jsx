...
const UserDetails = () => {
    ...
    useEffect(()=>{
         async function wishlist() {
            const response  = await axios.get(`${backend_route}/user/wishlist/${userEmail}`)
    ...
    <>
      ...
      <div className="flex w-full flex-col items-center gap-10 bg-gray-600 dark:bg-gray-800">
        ...
        <div className="text-white py-12 flex flex-col items-center">
            <div className="text-5xl text-violet-700 font-semibold my-3">Your Details</div>
           <div className="text-3xl text-gray-200">Name : <span className="text-2xl text-gray-100">{userName}</span></div>
           <div className="text-3xl text-gray-200">Email : <span className="text-2xl text-gray-100">{userEmail}</span></div>
           <div className="text-4xl text-violet-600 font-semibold my-3 mt-10">Your WishList</div>

           <div className="grid grid-cols-2 gap-6 mt-10 lg:mt-6 lg:gap-8 lg:grid-cols-4 px-6">
          {wishlist ? wishlist.map((details)=>{
              return  <Link to={`/aboutproduct?id=${details.Product.id}`}>
